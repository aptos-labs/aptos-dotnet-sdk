namespace Aptos.Tests.Core;

public sealed class SingleKeyTests
{
    private static SingleKey MakeSingleKey(string type, string key) =>
        new(
            type switch
            {
                "ed25519" => Ed25519PublicKey.Deserialize(new Deserializer(key)),
                "secp256k1" => Secp256k1PublicKey.Deserialize(new Deserializer(key)),
                "keyless" => KeylessPublicKey.Deserialize(new Deserializer(key)),
                _ => throw new ArgumentException("Invalid public key type"),
            }
        );

    public static TheoryData<string, string, string> SerializeData =>
        new()
        {
            {
                "ed25519",
                "0x2066e6dde4a271c5fcc68e9bda668ac41ff67049ea75e4db103c7927934b1e4ace",
                "0x002066e6dde4a271c5fcc68e9bda668ac41ff67049ea75e4db103c7927934b1e4ace"
            },
            {
                "secp256k1",
                "0x4104ffaf07a5268f92f86aa8be6a8047aaf2cceb33c3ac69628d9dde4e2ffdbe2d21551ecb9f69a6a79a509a7328a6641d81b4bb1fb39d4955549dad93d44ccc9d50",
                "0x014104ffaf07a5268f92f86aa8be6a8047aaf2cceb33c3ac69628d9dde4e2ffdbe2d21551ecb9f69a6a79a509a7328a6641d81b4bb1fb39d4955549dad93d44ccc9d50"
            },
            {
                "keyless",
                "0x1b68747470733a2f2f6163636f756e74732e676f6f676c652e636f6d20efbfb94b89da565468e11b1a7d29eb3be1032637ec8d21b61f5220511d668814",
                "0x031b68747470733a2f2f6163636f756e74732e676f6f676c652e636f6d20efbfb94b89da565468e11b1a7d29eb3be1032637ec8d21b61f5220511d668814"
            },
        };

    [Theory]
    [MemberData(nameof(SerializeData))]
    public void Serialize(string type, string key, string expectedBcs)
    {
        Assert.Equal(Hex.FromHexString(expectedBcs), MakeSingleKey(type, key).BcsToHex());
    }

    public static TheoryData<string, string, string> AuthenticationKeyData =>
        new()
        {
            {
                "ed25519",
                "0x2066e6dde4a271c5fcc68e9bda668ac41ff67049ea75e4db103c7927934b1e4ace",
                "0x73312812b63e652652f5a2aecfcb42ec051923c3456329d9500eb0e919e3903d"
            },
            {
                "secp256k1",
                "0x4104ffaf07a5268f92f86aa8be6a8047aaf2cceb33c3ac69628d9dde4e2ffdbe2d21551ecb9f69a6a79a509a7328a6641d81b4bb1fb39d4955549dad93d44ccc9d50",
                "0xe6e936c0c9c41d8c1f0490de291789677d64467ef38f1fff7111cad9546eee44"
            },
            {
                "keyless",
                "0x1b68747470733a2f2f6163636f756e74732e676f6f676c652e636f6d20efbfb94b89da565468e11b1a7d29eb3be1032637ec8d21b61f5220511d668814",
                "0x3c38e3250bf5e446947a9fd4bdfd41e76c3812e8061abf40702febde99b68d51"
            },
        };

    [Theory]
    [MemberData(nameof(AuthenticationKeyData))]
    public void DeriveAuthenticationKey(string type, string key, string expectedBcs)
    {
        Assert.Equal(Hex.FromHexString(expectedBcs), MakeSingleKey(type, key).AuthKey().BcsToHex());
    }

    public static TheoryData<string, string, string, string, bool> VerifySignatureData =>
        new()
        {
            {
                "ed25519",
                "0x2066e6dde4a271c5fcc68e9bda668ac41ff67049ea75e4db103c7927934b1e4ace",
                "0x40400461806f41581b75459e5db432e9d4da0881ac0ef0b7f8bb1c4b8e108d4ec2dd88abf5ff1e00616b2c338c375e245470e15d75f69aec465fa090001df19505",
                "hello world",
                true
            },
            {
                "ed25519",
                "0x2066e6dde4a271c5fcc68e9bda668ac41ff67049ea75e4db103c7927934b1e4ace",
                "0x40400461806f41581b75459e5db432e9d4da0881ac0ef0b7f8bb1c4b8e108d4ec2dd88abf5ff1e00616b2c338c375e245470e15d75f69aec465fa090001df19505",
                "N/A",
                false
            },
            {
                "secp256k1",
                "0x4104ffaf07a5268f92f86aa8be6a8047aaf2cceb33c3ac69628d9dde4e2ffdbe2d21551ecb9f69a6a79a509a7328a6641d81b4bb1fb39d4955549dad93d44ccc9d50",
                "0x40882b4ddbc14df439c222300c051d1039a62d36026ecd5789fd777224fef0ab7e07bb0891c5b411ec0938456a4b250067e8ff6c996d31f0186019521fc8f6fb25",
                "hello world",
                true
            },
            {
                "secp256k1",
                "0x4104ffaf07a5268f92f86aa8be6a8047aaf2cceb33c3ac69628d9dde4e2ffdbe2d21551ecb9f69a6a79a509a7328a6641d81b4bb1fb39d4955549dad93d44ccc9d50",
                "0x40882b4ddbc14df439c222300c051d1039a62d36026ecd5789fd777224fef0ab7e07bb0891c5b411ec0938456a4b250067e8ff6c996d31f0186019521fc8f6fb25",
                "N/A",
                false
            },
            {
                "keyless",
                "0x1b68747470733a2f2f6163636f756e74732e676f6f676c652e636f6d20efbfb94b89da565468e11b1a7d29eb3be1032637ec8d21b61f5220511d668814",
                "",
                "hello world",
                true
            },
            {
                "keyless",
                "0x1b68747470733a2f2f6163636f756e74732e676f6f676c652e636f6d20efbfb94b89da565468e11b1a7d29eb3be1032637ec8d21b61f5220511d668814",
                "",
                "N/A",
                false
            },
        };

    [Theory]
    [MemberData(nameof(VerifySignatureData))]
    public void VerifySignature(
        string type,
        string key,
        string signature,
        string message,
        bool expected
    )
    {
        var singleKey = MakeSingleKey(type, key);
        PublicKeySignature sig = type switch
        {
            "ed25519" => Ed25519Signature.Deserialize(new Deserializer(signature)),
            "secp256k1" => Secp256k1Signature.Deserialize(new Deserializer(signature)),
            "keyless" => KeylessSignature.Deserialize(new Deserializer(signature)),
            _ => throw new ArgumentException("Invalid signature type"),
        };
        Assert.Equal(expected, singleKey.VerifySignature(message, sig));
    }
}
