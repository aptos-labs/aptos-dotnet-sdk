...
public static class Ans
{
    ...
    {
        if (segment.Length < 3)
            throw new ANSInvalidName(segment, InvalidANSNameReason.InvalidLength);
        if (segment.Length > 63)
            throw new ANSInvalidName(segment, InvalidANSNameReason.InvalidLength);
        // only lowercase a-z and 0-9 are allowed, along with -. a domain may not start or end with a hyphen
        if (!Regex.IsMatch(segment, @"^[a-z\d][a-z\d-]{1,61}[a-z\d]$"))
            throw new ANSInvalidName(segment, InvalidANSNameReason.InvalidCharacters);
    }
    ...
    public static (string domain, string? subdomain) ParseAnsName(string name)
    {
        ...
        if (parts.Count > 2)
            throw new ANSInvalidName(name, InvalidANSNameReason.GreaterThanTwoParts);

        string? domain = parts.ElementAtOrDefault(0);
        if (domain == null)
            throw new ANSInvalidName(name, InvalidANSNameReason.MissingDomain);
        EnsureValidANSSegment(domain);

        string? subdomain = parts.ElementAtOrDefault(1);
        if (subdomain != null)
            EnsureValidANSSegment(subdomain);
