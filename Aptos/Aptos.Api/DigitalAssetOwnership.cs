...
public class TokenOwnership(
    decimal amount,
    DigitalAssetData currentTokenData,
    bool? isSoulboundV2,
    AccountAddress ownerAddress,
    decimal propertyVersionV1,
    string storageId,
    JsonElement? tokenPropertiesMutatedV1
)
...
)
{
    public DigitalAssetOwnership(IGetDigitalAssetOwnerships_Current_token_ownerships_v2 ownership)
        : this(
            ownership.Amount,
            ownership.Current_token_data != null
                ? new DigitalAssetData(ownership.Current_token_data)
                : throw new UnexpectedResponseException(
                    $"Expected DigitalAssetOwnership({ownership.Owner_address}) to have current token data"
                ),
...
public class DigitalAssetData(
    string description,
    bool? isFungibleV2,
    decimal? largestPropertyVersionV1,
    string tokenDataId,
    string tokenName,
    JsonElement? tokenProperties,
...
{
    public DigitalAssetData(IDigitalAssetData data)
        : this(
            data.Description,
            data.Is_fungible_v2,
            ...
            data.Current_collection != null
                ? new CollectionData(data.Current_collection)
                : throw new UnexpectedResponseException(
