...
{
    static readonly JsonSerializerSettings SpecifiedSubclassConversion = new() { ContractResolver = new SubclassSpecifiedConcreteClassConverter<TransactionSignature>() };
    ...
    public override TransactionSignature? ReadJson(JsonReader reader, Type objectType, TransactionSignature? existingValue, bool hasExistingValue, JsonSerializer serializer)
    {
        ...
        switch (type)
        {
            case "ed25519_signature":
                return JsonConvert.DeserializeObject<TransactionEd25519Signature>(jsonObject.ToString(), SpecifiedSubclassConversion);
            case "fee_payer_signature":
                return JsonConvert.DeserializeObject<TransactionFeePayerSignature>(jsonObject.ToString(), SpecifiedSubclassConversion);
            case "multi_agent_signature":
                return JsonConvert.DeserializeObject<TransactionMultiAgentSignature>(jsonObject.ToString(), SpecifiedSubclassConversion);
            case "single_sender":
                ...
                if (jsonObject.ContainsKey("public_key"))
                {
                    ...
                    if (publicKey?.StartsWith("0x") ?? false)
                    {
                        ...
                        signature = JsonConvert.DeserializeObject<AccountEd25519Signature>(jsonObject.ToString());
                    }
    ...
    public override void WriteJson(JsonWriter writer, TransactionSignature? value, JsonSerializer serializer) => serializer.Serialize(writer, value);
}


[Serializable]
public class TransactionEd25519Signature(Hex publicKey, Hex signature) : TransactionSignature(TransactionSignatureVariant.Ed25519)
{
...
[Serializable]
public class TransactionFeePayerSignature(AccountSignature sender, List<string> secondarySignerAddresses, List<AccountSignature> secondarySigners, string feePayerAddress, AccountSignature feePayerSigner) : TransactionSignature(TransactionSignatureVariant.FeePayer)
{
...
public class TransactionMultiAgentSignature(AccountSignature sender, List<string> secondarySignerAddresses, List<AccountSignature> secondarySigners) : TransactionSignature(TransactionSignatureVariant.MultiAgent)
{
...
public class TransactionSingleSenderSignature(AccountSignature accountSignature) : TransactionSignature(TransactionSignatureVariant.SingleSender)
{
