...
public class ANSClient(AptosClient client)
{
    ...
    public async Task<string?> GetAnsName(string address) => await GetAnsName(AccountAddress.From(address));
    ...
    public async Task<string?> GetAnsName(AccountAddress address)
    {
        ...
        if (routerAddress == null) throw new ANSUnsupportedNetwork();

        var response = await _client.Contract.View(new(
        ...
        var name = new List<string?> { subdomainName, domainName }.Where(s => s != null).ToList();
