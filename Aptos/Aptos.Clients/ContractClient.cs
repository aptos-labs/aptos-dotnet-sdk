...
public class ContractClient(AptosClient client)
{
    ...
    public async Task<MoveFunction?> GetFunctionAbi(string moduleAddress, string moduleName, string functionName)
    {
    ...
    public async Task<EntryFunctionAbi> GetEntryFunctionAbi(string moduleAddress, string moduleName, string functionName, ulong? ledgerVersion = null)
    {
        var functionAbi = await GetFunctionAbi(moduleAddress, moduleName, functionName);

        // If there's no ABI, then the function is invalid
        if (functionAbi == null) throw new ArgumentException($"Function not found for '{moduleAddress}::{moduleName}::{functionName}'");

        // Non-entry functions also can't be used
        if (functionAbi.IsEntry == false) throw new ArgumentException($"Function '{moduleAddress}::{moduleName}::{functionName}' is not an entry function");

        // Remove the signers argument
        int firstNonSignerIndex = Utilities.FindFirstNonSignerIndex(functionAbi);
        List<TypeTag> parameters = [];
        for (int i = firstNonSignerIndex; i < functionAbi.Parameters.Count; i++) parameters.Add(TypeTag.Parse(functionAbi.Parameters[i], true));

        return new EntryFunctionAbi(signers: firstNonSignerIndex, functionAbi.GenericTypeParams, parameters);
    }

    public async Task<ViewFunctionAbi> GetViewFunctionAbi(string moduleAddress, string moduleName, string functionName)
    {
        var functionAbi = await GetFunctionAbi(moduleAddress, moduleName, functionName);
        ...
        if (functionAbi.IsView == false) throw new ArgumentException($"Function '{moduleAddress}::{moduleName}::{functionName}' is not a view function");

        // Type tag parameters for the function
        List<TypeTag> parameters = [];
        for (int i = 0; i < functionAbi.Parameters.Count; i++) parameters.Add(TypeTag.Parse(functionAbi.Parameters[i], true));

        // Return types for the function
        List<TypeTag> returnTypes = [];
        for (int i = 0; i < functionAbi.Return.Count; i++) returnTypes.Add(TypeTag.Parse(functionAbi.Return[i], true));
    ...
    public async Task<List<object>> View(GenerateViewFunctionPayloadData data, ulong? ledgerVersion = null) => await View<List<object>>(data, ledgerVersion);
