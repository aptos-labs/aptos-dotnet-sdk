...
public class FaucetClient(AptosClient client)
{
    ...
    public async Task<UserTransactionResponse> FundAccount(string address, ulong amount) => await FundAccount(AccountAddress.From(address), amount);
    ...
    {
        AptosResponse<JObject> response = await _client.PostFaucet<JObject>(new(
            body: new { address = address.ToString(), amount },
            path: "fund",
            originMethod: "fundAccount"
        ));

        string? txnHash = JsonConvert.DeserializeObject<string[]>(response.Data["txn_hashes"]?.ToString() ?? "[]")?.FirstOrDefault();
        if (txnHash == null) throw new FaucetException("No transaction hash returned from faucet. The faucet may be down at the moment.");

        CommittedTransactionResponse txnResponse = await _client.Transaction.WaitForTransaction(txnHash);

        if (txnResponse is not UserTransactionResponse) throw new Exception("Unexpected transaction response from faucet");
