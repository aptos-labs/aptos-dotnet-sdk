...
public class KeylessClient(AptosClient client)
{
    ...
    public async Task<KeylessAccount> DeriveAccount(
        string jwt,
    ...
    {
        var response = await _client.PostPepper<PepperResponse>(
            new(
                body: new
                {
                    jwt_b64 = jwt,
    ...
    public async Task<ZeroKnowledgeSignature> GetProof(
        string jwt,
        EphemeralKeyPair ekp,
        byte[]? pepper = null,
        string? uidKey = "sub"
    )
    {
        ...
        if (pepper.Length != KeylessAccount.PEPPER_LENGTH)
            throw new ArgumentException(
                $"Pepper length in bytes should be {KeylessAccount.PEPPER_LENGTH}"
            );

        var (_, maxExpHorizonSecs) = await GetKeylessConfig();
        if (
            maxExpHorizonSecs
            < (ekp.ExpiryTimestamp - (ulong)DateTimeOffset.UtcNow.ToUnixTimeSeconds())
        )
            throw new ArgumentException(
                $"The EphemeralKeyPair is too long lived.  It's lifespan must be less than {maxExpHorizonSecs}"
            );

        var response = await _client.PostProver<ProverResponse>(
            new(
                body: new
                {
                    jwt_b64 = jwt,
        ...
        return new ZeroKnowledgeSignature(
            ...
            null,
            null,
            trainingWheelSignature: EphemeralSignature.Deserialize(
                new Deserializer(response.Data.TrainingWheelsSignature)
            )
        );
    }
    ...
    )> GetKeylessConfig(ulong? ledgerVersion = null)
    {
        return await Memoize.MemoAsync(
            async () =>
                (
                    await GetGroth16VerificationKey(ledgerVersion),
                    (await GetConfigurationResource(ledgerVersion)).MaxExpHorizonSecs
                ),
    ...
    public async Task<KeylessConfiguration> GetConfigurationResource(ulong? ledgerVersion = null)
    {
        ...
        return JsonConvert.DeserializeObject<KeylessConfiguration>(
            JsonConvert.SerializeObject(response.Data.Data)
        )!;
    }
    ...
    public async Task<Groth16VerificationKey> GetGroth16VerificationKey(ulong? ledgerVersion = null)
    {
        ...
        return JsonConvert.DeserializeObject<Groth16VerificationKey>(
            JsonConvert.SerializeObject(response.Data.Data)
        )!;
    }
}
