...
public class FungibleAssetClient(AptosClient client)
{
        ...
        if (value.Contains("::"))
        {
            ...
            else
            {
                ...
                [
                    value,
                    AccountAddress
                        .CreateObjectAddress(Constants.APTOS_FA_ADDRESS, value)
                        .ToStringLong(),
                ];
            }
        }
            ...
            else
            {
                try
                {
                    ...
                    [
                        formattedAddress,
                        (
                            await _client.View<ResourceStruct>(
                                new("0x1::coin::paired_coin", [formattedAddress])
                            )
                        ).ToString(),
                    ];
                }
    ...
    public async Task<List<FungibleAssetBalance>> GetAccountFungibleAssetBalances(
        AccountAddress address,
        int offset = 0,
        int limit = 50,
        current_fungible_asset_balances_bool_exp? where = null,
        current_fungible_asset_balances_order_by? orderBy = null
    ) => await GetAccountFungibleAssetBalances(address.ToString(), offset, limit, where, orderBy);
    ...
    /// <param name="orderBy">The order by condition of the query.</param>
    /// <returns>A list of fungible asset balances.</returns>
    public async Task<List<FungibleAssetBalance>> GetFungibleAssetBalances(
        current_fungible_asset_balances_bool_exp where,
        int offset = 0,
        int limit = 50,
        current_fungible_asset_balances_order_by? orderBy = null
    )
    {
        var response = await _client.Indexer.Query(async client =>
            await client.GetFungibleAssetBalances.ExecuteAsync(
                where,
                offset,
                limit,
        ...
        return response
            .Data.Current_fungible_asset_balances.Select(b =>
            {
                try
                {
