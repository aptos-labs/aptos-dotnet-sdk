...
public class DigitalAssetClient(AptosClient client)
{
    ...
    public async Task<List<DigitalAssetOwnership>> GetAccountDigitalAssetOwnerships(AccountAddress address, int offset = 0, int limit = 50, current_token_ownerships_v2_bool_exp? where = null, current_token_ownerships_v2_order_by? orderBy = null) => await GetAccountDigitalAssetOwnerships(address.ToString(), offset, limit, where, orderBy);
    ...
    public async Task<List<DigitalAssetOwnership>> GetAccountDigitalAssetOwnerships(string address, int offset = 0, int limit = 50, current_token_ownerships_v2_bool_exp? where = null, current_token_ownerships_v2_order_by? orderBy = null)
    {
    ...
    /// <param name="orderBy">The order by condition of the query.</param>
    /// <returns>A list of digital asset ownerships.</returns>
    public async Task<List<DigitalAssetOwnership>> GetDigitalAssetOwnerships(current_token_ownerships_v2_bool_exp where, int offset = 0, int limit = 50, current_token_ownerships_v2_order_by? orderBy = null)
    {
        var response = await _client.Indexer.Query(async client => await client.GetDigitalAssetOwnerships.ExecuteAsync(where, offset, limit, orderBy != null ? [orderBy] : []));
        return response.Data.Current_token_ownerships_v2.Select(b =>
        {
            try
            {
        ...
        }).Where(b => b != null).Cast<DigitalAssetOwnership>().ToList();
    }
