...
public class MultiKeyAccount : Account
{
    ...
    public MultiKeyAccount(MultiKey multiKey, List<Account> signers)
    {
        ...
        var signersAndBitPosition = signers.Select((signer, index) => new { signer, index }).ToList();
    ...
    public bool VerifySignature(string message, MultiKeySignature signature) => VerifySignature(SigningMessage.Convert(message), signature);
    ...
    public bool VerifySignature(byte[] message, MultiKeySignature signature)
    {
        ...
        for (int i = 0; i < signature.Signatures.Count; i++)
        {
            ...
            if (!singlePublicKey.VerifySignature(message, singleSignature)) return false;
        }
    ...
    public override Signature Sign(byte[] message) => new MultiKeySignature(Signers.Select(s => s.Sign(message)).ToList(), MultiKey.CreateBitmap(SignerIndicies));
    ...
    public override Signature SignTransaction(AnyRawTransaction transaction) => new MultiKeySignature(Signers.Select(s => s.SignTransaction(transaction)).ToList(), MultiKey.CreateBitmap(SignerIndicies));
    ...
    public override AccountAuthenticator SignWithAuthenticator(byte[] message) => new AccountAuthenticatorMultiKey(_publicKey, (MultiKeySignature)Sign(message));
    ...
    /// <param name="message">The message to sign as a byte array.</param>
    /// <returns>The authenticator containing the signature.</returns>
    public override AccountAuthenticator SignTransactionWithAuthenticator(AnyRawTransaction transaction) => new AccountAuthenticatorMultiKey(_publicKey, (MultiKeySignature)SignTransaction(transaction));
}