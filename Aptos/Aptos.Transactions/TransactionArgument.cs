...
{
    abstract public override void Serialize(Serializer s);

    abstract public void SerializeForScriptFunction(Serializer s);
    ...
    {
        ScriptTransactionArgumentVariants index = (ScriptTransactionArgumentVariants)d.Uleb128AsU32();
        return index switch
        {
            ...
            ScriptTransactionArgumentVariants.U8Vector => MoveVector<U8>.Deserialize(d, U8.Deserialize),
            _ => throw new ArgumentException($"Unknown variant index for ScriptTransactionArgument: {index}"),
        };
    }
    ...
    public static TransactionArgument? Parse(object? value, TypeTag typeTag, List<TypeTag> genericTypeParams)
    {
        ...
        {
            if (value is bool boolValue) return new Bool(boolValue);
            if (value is string boolStringValue)
            {
                if (boolStringValue == "false") return new Bool(false);
                if (boolStringValue == "true") return new Bool(true);
            }
        }

        if (typeTag is TypeTagAddress)
        {
            if (value is string addressStringValue) return AccountAddress.FromString(addressStringValue);
        }

        if (typeTag is TypeTagU8)
        {
            if (value is byte u8Value) return new U8(u8Value);
            if (value is string u8StringValue) return new U8(byte.Parse(u8StringValue));
        }

        if (typeTag is TypeTagU16)
        {
            if (value is ushort u16Value) return new U16(u16Value);
            if (value is string u16StringValue) return new U16(ushort.Parse(u16StringValue));
        }

        if (typeTag is TypeTagU32)
        {
            if (value is uint u32Value) return new U32(u32Value);
            if (value is string u32StringValue) return new U32(uint.Parse(u32StringValue));
        }

        if (typeTag is TypeTagU64)
        {
            if (value is ulong u64Value) return new U64(u64Value);
            if (value is string u64StringValue) return new U64(ulong.Parse(u64StringValue));
        }

        if (typeTag is TypeTagU128)
        {
            if (value is BigInteger u128Value) return new U128(u128Value);
            if (value is string u128StringValue) return new U128(BigInteger.Parse(u128StringValue));
        }

        if (typeTag is TypeTagU256)
        {
            if (value is BigInteger u256Value) return new U256(u256Value);
            if (value is string u256StringValue) return new U256(BigInteger.Parse(u256StringValue));
        }
        ...
        if (typeTag is TypeTagVector typeTagVector)
        {
            ...
            if (value is IEnumerable list)
            {
                ...
                {
                    convertedList.Add(ConvertArgument(item, typeTagVector.Value, genericTypeParams));
                }
        ...
        if (typeTag is TypeTagStruct typeTagStruct)
        {
            ...
            {
                if (value is string stringValue) return new MoveString(stringValue);
            }

            if (typeTagStruct.IsObjectTypeTag())
            {
                if (value is string stringValue) return AccountAddress.FromString(stringValue);
            }
            ...
            if (typeTagStruct.IsOptionTypeTag())
            {
                if (value is null)
                {
                    ...
                    if (innerTypeTag is TypeTagBool) return new MoveOption<Bool>(null);
                    if (innerTypeTag is TypeTagU8) return new MoveOption<U8>(null);
                    if (innerTypeTag is TypeTagU16) return new MoveOption<U16>(null);
                    if (innerTypeTag is TypeTagU32) return new MoveOption<U32>(null);
                    if (innerTypeTag is TypeTagU64) return new MoveOption<U64>(null);
                    if (innerTypeTag is TypeTagU128) return new MoveOption<U128>(null);
                    if (innerTypeTag is TypeTagU256) return new MoveOption<U256>(null);
                ...
                return new MoveOption<TransactionArgument>(ConvertArgument(value, typeTagStruct.Value.TypeArgs[0], genericTypeParams));
            }
        }
