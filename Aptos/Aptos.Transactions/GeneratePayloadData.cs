...
public abstract class GenerateTransactionPayloadDataWithAbi(
    string function,
    List<object>? functionArguments,
    List<object>? typeArguments = null,
    EntryFunctionAbi? abi = null
) : GeneratePayloadData(typeArguments)
{
    /// <summary>
...
public class GenerateViewFunctionPayloadData(
    string function,
    List<object>? functionArguments,
    List<object>? typeArguments = null,
    ViewFunctionAbi? abi = null
) : GeneratePayloadData(typeArguments)
{
...
/// <param name="typeArguments">A list of <see cref="TypeTag"/> objects representing the type arguments of the function.</param>
/// <param name="abi">The ABI of the function that will be used to check the type and function arguments of the transaction.</param>
public class GenerateEntryFunctionPayloadData(
    string function,
    List<object>? functionArguments = null,
    List<object>? typeArguments = null,
    EntryFunctionAbi? abi = null
) : GenerateTransactionPayloadDataWithAbi(function, functionArguments, typeArguments, abi)
{
    public GenerateEntryFunctionPayloadData(
        GenerateEntryFunctionPayloadData data,
        EntryFunctionAbi? abi = null
    )
        : this(data.Function, data.FunctionArguments, data.TypeArguments, abi) { }
}

public class GenerateMultisigPayloadData(
    string function,
...
) : GenerateTransactionPayloadDataWithAbi(function, functionArguments, typeArguments, abi)
{
    ...
    public GenerateMultisigPayloadData(
        string function,
        ...
        : this(
            function,
            AccountAddress.FromString(multisigAddress),
            functionArguments,
            typeArguments,
            abi
        ) { }
...
public class GenerateScriptPayloadData(
    byte[] bytecode,
    List<IScriptFunctionArgument>? functionArguments = null,
    List<object>? typeArguments = null
) : GeneratePayloadData(typeArguments)
{
    /// <summary>
