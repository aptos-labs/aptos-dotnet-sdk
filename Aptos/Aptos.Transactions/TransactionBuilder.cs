...
public static class TransactionBuilder
{
    ...
    public static SignedTransaction GeneratedSignedTransaction(SubmitTransactionData data)
    {
        ...
        {
            if (data.FeePayerAuthenticator == null) throw new ArgumentException("FeePayerAuthenticator is required for transactions that have a FeePayer address");
        ...
        {
            transactionAuthenticator = new TransactionAuthenticatorEd25519(ed25519Authenticator.PublicKey, ed25519Authenticator.Signature);
        }
    ...
    {
        AccountAuthenticator senderAuthenticator = GetAuthenticatorForSimulation(data.SignerPublicKey);

        if (data.Transaction.FeePayerAddress != null)
        {
            if (data.FeePayerPublicKey == null) throw new ArgumentException("FeePayerPublicKey is required for transactions that have a FeePayer address");
            ...
            return new SignedTransaction(transactionToSign.RawTransaction, feePayerTransactionAuthenticator);
        }

        if (data.Transaction.SecondarySignerAddresses != null)
        {
            if (data.Transaction.SecondarySignerAddresses.Count > 0 && data.SecondarySignersPublicKeys == null) throw new ArgumentException("SecondarySignersPublicKeys is required for transactions that have secondary signer addresses");
        ...
        {
            AccountAuthenticatorEd25519 ed25519Authenticator => new TransactionAuthenticatorEd25519(ed25519Authenticator.PublicKey, ed25519Authenticator.Signature),
    ...
    public class GenerateTransactionOptions(ulong? maxGasAmount = null, ulong? gasUnitPrice = null, ulong? expireTimestamp = null, ulong? accountSequenceNumber = null)
    {
    ...
    public static async Task<SimpleTransaction> GenerateTransaction(AptosClient client, AccountAddress sender, GenerateEntryFunctionPayloadData data, bool withFeePayer = false, GenerateTransactionOptions? options = null)
    {
    ...
    public static async Task<SimpleTransaction> GenerateTransaction(AptosClient client, AccountAddress sender, GenerateScriptPayloadData data, bool withFeePayer = false, GenerateTransactionOptions? options = null)
    {
        var payload = GenerateTransactionPayload(client, data);
        return await BuildRawTransaction(client, payload, sender, withFeePayer, options);
    }
    public static async Task<MultiAgentTransaction> GenerateTransaction(AptosClient client, AccountAddress sender, GenerateEntryFunctionPayloadData data, List<AccountAddress> secondarySignerAddresses, bool withFeePayer = false, GenerateTransactionOptions? options = null)
    {
        var payload = await GenerateTransactionPayload(client, data);
        return await BuildRawTransaction(client, payload, sender, secondarySignerAddresses, withFeePayer, options);
    }
    ...
    public static TransactionScriptPayload GenerateTransactionPayload(AptosClient _, GenerateScriptPayloadData data) => new(new(data.Bytecode, Utilities.StandardizeTypeTags(data.TypeArguments ?? []), data.FunctionArguments ?? []));
    public static TransactionPayload GenerateTransactionPayload(AptosClient client, GenerateMultisigPayloadData data) => throw new NotImplementedException();
    public static async Task<TransactionEntryFunctionPayload> GenerateTransactionPayload(AptosClient client, GenerateEntryFunctionPayloadData data)
    {
        (string moduleAddress, string moduleName, string functionName) = Utilities.ParseFunctionParts(data.Function);

        EntryFunctionAbi functionAbi = data.Abi ?? await Memoize.MemoAsync(
            async () => await client.Contract.GetEntryFunctionAbi(moduleAddress, moduleName, functionName),
            $"entry-function-abi-{moduleAddress}-{moduleName}-{functionName}",
            1000 * 60 * 5 // 5 minutes
        )();

        EntryFunction payload = EntryFunction.Generate(data.Function, data.FunctionArguments ?? [], data.TypeArguments ?? [], functionAbi);
        return new TransactionEntryFunctionPayload(payload);
    }

    public static async Task<EntryFunction> GenerateViewFunctionPayload(AptosClient client, GenerateViewFunctionPayloadData data)
    {
        (string moduleAddress, string moduleName, string functionName) = Utilities.ParseFunctionParts(data.Function);

        ViewFunctionAbi functionAbi = data.Abi ?? await Memoize.MemoAsync(
            async () => await client.Contract.GetViewFunctionAbi(moduleAddress, moduleName, functionName),
        ...
        return EntryFunction.Generate(data.Function, data.FunctionArguments ?? [], data.TypeArguments ?? [], functionAbi);
    }
    ...
    public static async Task<MultiAgentTransaction> BuildRawTransaction(AptosClient client, TransactionPayload payload, AccountAddress sender, List<AccountAddress> secondarySignerAddresses, bool withFeePayer = false, GenerateTransactionOptions? options = null)
    {
        AccountAddress? feePayerAddress = withFeePayer ? AccountAddress.ZERO : null;
        RawTransaction rawTxn = await GenerateRawTransaction(client, sender, payload, feePayerAddress, options);
        return new MultiAgentTransaction(rawTxn, secondarySignerAddresses, feePayerAddress);
    }
    public static async Task<SimpleTransaction> BuildRawTransaction(AptosClient client, TransactionPayload payload, AccountAddress sender, bool withFeePayer = false, GenerateTransactionOptions? options = null)
    {
        AccountAddress? feePayerAddress = withFeePayer ? AccountAddress.ZERO : null;
        RawTransaction rawTxn = await GenerateRawTransaction(client, sender, payload, withFeePayer ? AccountAddress.ZERO : null, options);
        return new SimpleTransaction(rawTxn, feePayerAddress);
    }

    public static async Task<RawTransaction> GenerateRawTransaction(AptosClient client, AccountAddress sender, TransactionPayload payload, AccountAddress? feePayerAddress, GenerateTransactionOptions? options = null)
    {

        async Task<byte> GetChainId()
        {
            if (client.Config.NetworkConfig.ChainId == -1) return (await client.Block.GetLedgerInfo()).ChainId;
            return (byte)client.Config.NetworkConfig.ChainId;
        }

        async Task<ulong> GetGasUnitPrice()
        {
            if (options?.GasUnitPrice != null) return options.GasUnitPrice.Value;
            return (await client.Gas.GetGasPriceEstimation()).GasEstimate;
        }

        async Task<ulong> GetSequenceNumber()
        {
            if (options?.AccountSequenceNumber != null) return options.AccountSequenceNumber.Value;
            ...
            catch (Exception e)
            {
                ...
                if (feePayerAddress != null && AccountAddress.From(feePayerAddress).Equals(AccountAddress.ZERO))
                {
                    return 0;
                }
                throw e;
        ...
        var (chainIdTask, gasUnitPriceTask, sequenceNumberTask) = (GetChainId(), GetGasUnitPrice(), GetSequenceNumber());
        ...
        return new RawTransaction(
            ...
            gasUnitPrice,
            expirationTimestampSecs: options?.ExpireTimestamp ?? (ulong)DateTime.Now.ToUnixTimestamp() + Constants.DEFAULT_TXN_EXP_SEC,
            chainId
        );
