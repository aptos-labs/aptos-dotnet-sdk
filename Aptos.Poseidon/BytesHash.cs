...
public static class BytesHash
{
    ...
    private static readonly int _maxNumInputBytes =
        (_maxNumInputScalars - 1) * _bytesPackedPerScalar;

    public static BigInteger HashWithLength(string str, int maxSizeBytes) =>
        HashWithLength(Encoding.UTF8.GetBytes(str), maxSizeBytes);

    public static BigInteger HashWithLength(byte[] bytes, int maxSizeBytes)
    {
        if (bytes.Length > maxSizeBytes)
            throw new ArgumentException(
                $"Input bytes of length {bytes.Length} is longer than {maxSizeBytes} bytes"
            );
    ...
    public static List<BigInteger> PackBytes(byte[] bytes)
    {
        if (bytes.Length > _maxNumInputBytes)
            throw new ArgumentException($"Can't pack more than {_maxNumInputBytes} bytes");
    ...
    {
        if (paddedSize < bytes.Length)
            throw new ArgumentException(
                $"Padded size {paddedSize} must be greater than or equal to the input array size ({bytes.Length})"
            );
